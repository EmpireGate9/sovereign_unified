...
def send_message(body: MessageCreate, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
...
@router.get("/history")
def history(project_id: int, limit: int = 50, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
